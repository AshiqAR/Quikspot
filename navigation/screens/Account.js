import { View, Text, Pressable, Modal, ActivityIndicator, StyleSheet, TouchableWithoutFeedback } from 'react-native'
...
export default function Account({ navigation }) {
    ...
    if (!isScrolling) {
      ...
      switch (action) {
        ...
        case 'RentSpaceAndEarn':
          navigation.navigate('RentASpaceNavigatorStack');
          break;
        case 'Legal':
  ...
  return (
    <View style={{ flex: 1, backgroundColor: 'white' }}>
      ...
      <View style={{ height: 40, width: 40, marginTop: 20, marginHorizontal: 20 }}>
        <Pressable
          style={{}}
          onPress={handleClose}
      ...
      <Pressable
        style={{ flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 30, paddingVertical: 10, alignItems: 'center' }}
        onPress={() => handlePress('ManageMyAccount')}
      >
        <Text style={{ fontSize: 30, fontWeight: 'bold', color: 'black' }}>{user.name}</Text>
      ...
      >
        ...
        <ActivitySection navigation={navigation} isScrolling={isScrolling}></ActivitySection>

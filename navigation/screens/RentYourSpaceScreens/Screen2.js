...
export default function Screen2({ navigation }) {
    ...
    const handleNextPress = () => {
        ...
        if (
            isNaN(estimatedCarsParked) || estimatedCarsParked <= 0 ||
            isNaN(pricePerHour) || pricePerHour <= 0 ||
            parkAreaDetails.parkSpaceType == ""
        ) {
            console.log(isNaN(estimatedCarsParked), isNaN(pricePerHour), parkAreaDetails.parkSpaceType == "");
    ...
    const handleToggleFacility = (index) => {
        const updatedFacilities = parkAreaDetails.facilitiesAvailable.map((facility, i) => {
            if (i === index) {
                return { ...facility, value: !facility.value };
            }
            return facility;
        });
        updateParkAreaDetails.updateDetails('facilitiesAvailable', updatedFacilities);
    };
    ...
    const renderParkSpaceOptions = () => {
        return parkSpaceOptions.map((option, index) => (
            <Pressable key={index} style={styles.radioContainer} onPress={() => handleInputChange('parkSpaceType', option)}>
                <Icon name={parkAreaDetails.parkSpaceType === option ? 'radio-button-on' : 'radio-button-off'} size={20} color={'black'} />
        ...
        <View style={styles.container}>
            <ScrollView style={styles.scrollSection}>
                ...
                <LabelInput
                    ...
                    onChangeText={(value) => handleInputChange('expectedPricePerHour', value)}
                    keyboardType="numeric"
                />
                <View style={styles.inputContainer}>
                    <Text style={styles.label}>Type of Park Space*</Text>
                    <View style={styles.radioGroup}>
                        {renderParkSpaceOptions()}
                    </View>
                </View>
                <View style={[{ alignItems: 'center', paddingVertical: 10 }]}><Text style={styles.label}>Park Space Facilities Available</Text></View>

                {renderFacilityToggles()}

                <View style={[styles.label, { alignItems: 'center', paddingVertical: 10 }]}><Text style={styles.label}>Locate the parking area</Text></View>
                <Pressable
                    style={{ flexDirection: 'row', alignItems: 'center', marginVertical: 10, backgroundColor: '#dfdfdf', padding: 10, borderRadius: 5 }}
                    onPress={handleLocationFetch}
                >
                    <Icon name="location-outline" size={24} color="gray" />
                    <Text style={{ marginLeft: 8, color: 'gray' }}>Use Current Location</Text>
                </Pressable>
...
const LabelToggle = ({ label, value, onToggle }) => (
    <Pressable onPress={onToggle} style={styles.toggleContainer}>
        <Text style={styles.label}>{label}</Text>
        <Icon name={value ? 'checkbox-outline' : 'square-outline'} size={24} color={'black'} />
    </Pressable>
);
